...
class LocationtypesController extends BaseController
{
    var $custRepoObj;
    ...
    public function index()
    {
        ...
        $customers = $this->custRepoObj->getAllCustomers();
         ...
         $customers = array();        
         $locas = DB::table('location_types')->where(array('manufacturer_id'=>Session::get('customerId'),'is_deleted'=>0))->lists('location_type_name','location_type_id');   
      }  
               $customerId = Session::get('customerId');
        ...
        return View::make('locationtypes/view')->with(['allow_buttons' => ['add' => $allowAddCustomer],'customer_id'=>$customer_id,'customerbusinessid'=>$customerbusinessid,'formData' => $formData,'customerstoragelocations'=>$customerstoragelocations,'locas'=>$locas,'customers'=>$customers,'customerId'=>$customerId,'customer_locations' => $customerlocations]);
...
//                        echo 'checkLocation => '.$checkLocation;
    ...
    public function getTreeLocations($id)
    {
        ...
        $locas = DB::Table('location_types')
                ...
                ->where('location_types.location_type_id',$location_type_id)
                ->get();
//dd($location_type_id); die;
        $finalCustArrs = array();
        $custs = array();
...
/*                $cust['longitude'] = $subloc['longitude'];
         ...
         $endTime = $this->getTime();
                DB::table('user_tracks')->insert(array('user_id'=>Session::get('userId'),'manufacturer_id'=>Session::get('customerId'),'message'=>'Successfully retrieved.','service_name'=>'Gettreelocatoins','status'=>1,'response_duration'=>($endTime - $startTime)));
    ...
    public function saveLocation()
    {
        ...
        //dd($data);
        $startTime = $this->getTime();
        $manuId=$this->roleRepo->decodeData($data['manufacturer_id']);
        $stateName = DB::Table('zone')->select('name')->where('zone_id',$data['state'])->get();
        ...
        $endTime = $this->getTime();
                DB::table('user_tracks')->insert(array('user_id'=>Session::get('userId'),'manufacturer_id'=>Session::get('customerId'),'message'=>'Successfully Created.','service_name'=>'save location','status'=>1,'response_duration'=>($endTime - $startTime)));
        return Response::json([
                    'status' => true,
                    'message' => 'Sucessfully added.'
        ]);
    ...
    public function editLocation($locationId)
    {
        ...
        $endTime = $this->getTime();
                DB::table('user_tracks')->insert(array('user_id'=>Session::get('userId'),'manufacturer_id'=>Session::get('customerId'),'message'=>'Successfully edited.'.json_encode($locationId),'service_name'=>'edit location','status'=>1,'response_duration'=>($endTime - $startTime)));
    ...
    $startTime = $this->getTime();
        DB::Table('locations')
            ...
            'modified_by'=>Session::get('userId')
                ...
                $endTime = $this->getTime();
                DB::table('user_tracks')->insert(array('user_id'=>Session::get('userId'),'manufacturer_id'=>Session::get('customerId'),'message'=>'Successfully updated.','service_name'=>'update location','status'=>1,'response_duration'=>($endTime - $startTime)));
        return Response::json([
                    'status' => true,
    ...
    $startTime = $this->getTime();
        ...
        $endTime = $this->getTime();
                DB::table('user_tracks')->insert(array('user_id'=>Session::get('userId'),'manufacturer_id'=>Session::get('customerId'),'message'=>'Successfully delted.','service_name'=>' delete location','status'=>1,'response_duration'=>($endTime - $startTime)));
        return Response::json([
            'status' => true,
            'message' => 'Sucessfully deleted.'
        ]);
    ...
    public function restoreLocation($locationId)
    {
        ...
        $endTime = $this->getTime();
                DB::table('user_tracks')->insert(array('user_id'=>Session::get('userId'),'manufacturer_id'=>Session::get('customerId'),'message'=>'Successfully restored.','service_name'=>'restore location','status'=>1,'response_duration'=>($endTime - $startTime)));
        return Response::json([
            'status' => true,
            'message' => 'Sucessfully Restored.'
        ]);
...
public function locationtypes($id){
    return DB::table('location_types')->where(array('manufacturer_id'=>$id,'is_deleted'=>0))->lists('location_type_name','location_type_id');


}

public function locationaddressconfig(){

    parent::Breadcrumbs(array('Home' => '/', 'MRP Label Template' => 'addressconfig/locations'));

    $location_types = DB::table('location_types')->where('manufacturer_id','=',5)->whereIn('location_type_name',array('Supplier','Plant','Depot','Service Center'))->get();
    return View::make('locationtypes/addressconfig')->with('location_types',$location_types);

}

public function getlocationwisedata($val){
    ...
    //echo "i am ahhh";exit;
        ...
        $concatQuery="";
              ...
              $concatQuery =  "CONCAT('<a href=\"javascript:void(0)\" onclick=\"updateeditaddressdata(',l.location_id,')\">
                    <span class=\"badge bg-grey\"><i class=\"fa fa-edit\"></i></span></a>
              <span style=\"padding-left:10px;\" ></span>') as actions";
        


        $details ="select l.print_address,l.location_id,l.location_name,l.erp_code,".$concatQuery." from locations as l join location_types as lt on l.location_type_id=lt.location_type_id where lt.location_type_id=".$val." and lt.manufacturer_id=5";
        $tot = DB::select(DB::raw($details));
        /*$return=[];
        foreach ($tot as $key => $value) {

            $temp=[];
            $temp['location_name']=$value->erp_code.'-'.$value->location_name;
            $temp['actions']=$value->actions;
            $value->print_address=json_decode($value->print_address);
            
            if(isset($value->print_address->locationtype))
            $temp['locationtype']=$value->print_address->locationtype;
            else 
            $temp['locationtype']='';

            if(isset($value->print_address->labels)){

           // echo "<pre/>";print_r($value->print_address->labels);
	        	foreach ($value->print_address->labels as $k => $v) {
	        		$templbl='';
	        		foreach ($v as $k1 => $v1) {
	        			if($k1=='label')
	        				continue;
	        		...
	        		$temp['print_address_'.$v->label]=$templbl;        		
	        	}
        	}
        	else {
        		$temp['print_address_3X4']='-';
        		$temp['print_address_4X3']='-';
        		$temp['print_address_4X6.5']='-';
        	}
        	$return[]=$temp;
        }*/
        //exit;
...
public function updateprintaddress(){
    $dd = Input::all();

    //echo "<pre/>";print_r($dd);exit;
    $ddddd =  json_encode($dd['input']);
    if($dd['loc_update_id'] !=0){
        $update = "update locations set print_address='".$ddddd."' where location_id=".$dd['loc_update_id']."";
        $tot = DB::select(DB::raw($update));
...
public function ExporttoExcelAddress($id){


    $data1 = DB::table('locations')->select('erp_code','location_name','print_address')->where('location_type_id','=',$id)->get();
...
public function getdataforprintaddress($id){

    $data = DB::table('locations')->where('location_id','=',$id)->pluck('print_address');
...
public function ExportLabeldataaddress(){

$headers1=array('Erp Code','Location Name','3X4 line1','3X4 line2','3X4 line3','3X4 line4','3X4 line5','4X3 line1','4X3 line2','4X3 line3','4X3 line4','4X3 line5','4X6.5 line1','4X6.5 line2','4X6.5 line3','4X6.5 line4','4X6.5 line5','4X6.5 line6');

$headers=array('Label Name','3X4','','','','','4X3','','','','','4X6.5','','','','','');
$iot_array=array();

     ob_end_clean(); 
        Excel::create('Label Template-', function($excel) use($headers,$headers1,$iot_array) 
            {
                $excel->sheet("Template", function($sheet) use($headers,$headers1,$iot_array)
                {
                	$sheet->mergeCells('A2:B2');
                	$sheet->mergeCells('C2:G2');
                	$sheet->mergeCells('H2:L2');
                	$sheet->mergeCells('M2:R2');
                    $sheet->loadView('locationtypes.downloadaddressTemplate', array('headers_one' => $headers1,'headers_two'=>$headers));
...
public function importLabeladdressdata(){

    $file_data    =  Input::file('template_address_data');
      $file_extension  = $file_data->getClientOriginalExtension();
      $msg = "";


      // echo "<pre/>";print_r('testsss');exit;

       $data = Excel::load($file_data, function($reader) {
            })->get();
       $i=0;
       ...
       foreach ($data  as $key => $value) {
       		...
       		$temp=[];
       		$temp['locationtype']=$value->location_name;
       		$temp['erp_code']=$value->erp_code;

       		//echo $value->erp_code;exit;

       		//check erp_code exist or not

       		$erp_codecheck=DB::table('locations')->where('erp_code','=',$value->erp_code)->where('manufacturer_id','=',5)->count();


       //echo $erp_codecheck;exit;

       		if($erp_codecheck >=1){
       		$templblTypes=['3X4'=>0,'4X3'=>1,'4X6.5'=>2];
       		$ii=0;
       		foreach ($templblTypes as $lkey => $lvalue)
       			$temp['labels'][$lvalue]['label']=$lkey;
       		...
       		foreach ($value as $ikey => $ivalue) {
       			...
       			if($ii<=2) continue;
       			$dataj=explode('_',$ikey);
       			if($dataj[0]){
       			$llbindex=str_replace('4X65','4X6.5',strtoupper($dataj[0]));
       			$temp['labels'][$templblTypes[$llbindex]][$dataj[1]]=$ivalue;
       			}
       		}

       		
//echo "<pre/>";print_r($value->erp_code);exit;

       		$query =  "update locations as l set l.print_address='".json_encode($temp)."' where l.erp_code=".$temp['erp_code']."";
       		//exit;
       	$tot = DB::select(DB::raw($query));

       	return 'Sheet Uploaded succesfully';
       }else{
       	return 'error in Excel sheet';
       }
