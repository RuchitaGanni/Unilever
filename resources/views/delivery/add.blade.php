...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/4.0.12/js/bootstrap-select.min.js"></script>
 <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.js"></script>
           ...
           <div class="row">
            <!-- <div class="form-group col-sm-3">
              ...
              <div class="form-group col-sm-3">
                  <label>Item</label>
                    <div id="location">
                      ...
                      <select class="form-control  " data-live-search="true" id="item" name="item" 
                       required="required" >
              ...
              <div class="form-group col-sm-2">
               ...
               <!-- <div class="form-group col-sm-2" id="test_ruchi">
                <label>Manual Batch</label>
                <br>
                <input type="checkbox" name="mBatch" id="mBatch">
            </div >  -->
          <!-- <button type="button"  id="reset_batch" class="btn-primary pull-right" style="margin-left:170px;" >Reset</button> -->
            <div><label style="margin-left:20px;">Batch no<a id="reset_batch" style="margin-left:188px;color: blue;">Reset</a></label>
            <div class="BatchDrop">
              <div class="form-group col-sm-3">
                
               <!--  <input type="text" class="form-control"  id="bno" name="bno"> -->
                 <!-- <select class="form-control batch"  id="bno" name="bno" required="required"> -->
                  <select class="form-control batch" id="bno" name="bno" parsley-trigger="change" parsley-required="true" parsley-error-container="#selectbox">
            ...
            <div class="BatchEntry">
              ...
              <div class="form-group col-sm-1">
                <button type="button" id="add_assign" class="btn btn-primary" style="margin-top:-2px;">
...
$('#item').change(function(){
  ...
  //var x=$('<label><input type="checkbox" name="" value="" />hey ruchi</label><br/>');
  //$('.batch').append('<option value="0">"'+x+'"</option>');
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      //$('.batch').append('<option value="0">Please select batch</option>');
      data=JSON.parse(data);
                // var myDiv = document.getElementById("mBatch")(); 
                 //var myDiv = $("#mBatch").html(); 
                 //var myDiv = $("#test_ruchi").html(); 
                //$('.batch').append('<option value="0">Please select batch</option>');

                  $('.batch').append('<option value="other" style="font-weight: bold;">MANUAL ENTRY</option>');
                 // $('.batch').append('<option>'+myDiv+'</option');
                $.each( data, function( key, value ){ 
                 // $('.batch').append('<label><input type="checkbox" name="" value="" />hey ruchi</label><br/>')
...
/*------checking if batch is entry or dropdown-------*/
/*$('#mBatch').click(function(){
  if($('#mBatch').prop('checked')==true){
       $('.BatchDrop').toggle();
       $('.BatchEntry').show();
...
var textToAdd  = document.getElementById("bn").value;
  ...
  $('#type').change(function(){
    ...
    $('#reset_batch').click(function (){
     
       $('.BatchDrop').show();
       $('.BatchEntry').hide();
          $('#bnoE').val('');
          //$('#bno').prop('selectedIndex',0);
          $("#bno").select2("val", "0");
...
$('#add_assign').click(function(){
            ...
            var b_noo= $('#bno').val();
            var b_no= $('#bno').val();
            if(b_noo==0 || b_noo==''||b_noo=='other'){
              b_noo=$('#bnoE').val();
              b_no=$('#bnoE').val();
            }
            /*$('#mBatch').prop('checked')==true  
              alert('test');
            var b_noo= $('#bno').val();
            var b_no= $('#bno').val();
            alert(b_noo);
