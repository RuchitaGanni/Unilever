    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" id='tp_data_title'>IOT details</h4>
             
        </div>
        <div class="modal-body">
          <button class="btn btn-primary" style="float: right" id="export_Excel">Download CSV!</button>
          <br><br>
          <div id='tp_list_grid'></div>
        </div>
      </div>
    </div>
  </div>
          ...
          <div class="form-group">
            <label>Material</label>
              <select class="form-control productSelect  selectpicker" data-live-search="true" id="product" name="product" required="required" multiple="multiple" >
               <option value="" disabled="true">Please Select one Product</option>
          ...
          <div class="form-group">
            <label>Plant</label>
              <select class="form-control vendorSelect selectpicker" data-live-search="true" id="vendor" name="vendor" required="required">
               <option value="0" disabled="true" selected="selected">Please Select one Location</option>
               @foreach($locations as $location)
                  <option value="{{$location->location_id}}"> {{$location->location_name}}-{{$location->erp_code}}</option>
                @endforeach
              </select>
          </div>
        </div>
...
function cancelPo(){
     ...
     var x = document.getElementById("loader");
        var x1 = document.getElementById("pocancel");
...
//      alert(data);
 ...
 // var doc_num=doc.slice(11, 25);
  var doc_num=doc.split(":")[1];
        $("#tp_list_grid").jqxGrid('exportdata', 'csv', 'ProcessOrderReport -'+doc_num);
    });
...
//   }
// });
    function getIOT(order_no) {
            // $('#iot').empty(); 
            // $('#iot_list_grid').hide();
                $('#getIOT').modal('toggle');
                var  url="/reports/iotgrid/"+order_no;
                 $('#tp_data_title').empty(); 
                var div = document.getElementById('tp_data_title');
              
                div.innerHTML += 'PO Number: '+order_no;
               $.ajax({
                    type: "get",
                    url:url,
            ...
            var  cols1=[
            { text: 'IOT',  datafield: 'primary_id',width:'15%',width: 250 },
            { text: 'Batch Number',  datafield: 'batch_no',width:150},
            { text: 'Product Name',  datafield: 'product_name',width:150},
            { text: 'Category Name',  datafield: 'category_name',width:150},
            { text: 'Location Name',  datafield: 'location_name',width:150},
            { text: 'Material Code',  datafield: 'material_code',width:120},
            { text: 'Mfg Date',  datafield: 'mfg_date',width:120},
            { text: 'ERP Code',  datafield: 'erp_code',width:100},
            { text: 'EAN No',  datafield: 'ean',width:150},
            { text: 'MRP',  datafield: 'mrp',width:100},
            { text: 'Quantity',  datafield: 'pkg_qty',width:100}
...
getdetailsforpo=function ()
{
  ...
  var  url="productorder/getPOorders_report/"+p_id+"/"+l_id;
 ...
 var  columns=[
                { text: 'Order No', datafield: 'order_no',width:150 },
                ...
                { text: 'EAN', datafield: 'ean',width:150},
                { text: 'Quantity', datafield: 'qty',width:250},
                { text: 'Packed Qty', datafield: 'packed_qty',width:100},
                { text: 'Type', datafield: 'po_type',width:50},
                { text: 'Status', datafield: 'po_status',width:50},
                // { text: 'UOM', datafield: 'order_uom',width:'10%'},
                { text: 'Created Date', datafield: 'date',width:195}
